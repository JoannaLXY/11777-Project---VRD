...
def get_depth_feats(depth, sub_box, obj_box, depth_size):
    ...
    # get union box
    uni_box = _union(sub_box, obj_box)

    # float to int
    sub_box = sub_box.astype(np.int32)
    obj_box = obj_box.astype(np.int32)
    uni_box = uni_box.astype(np.int32)

    # cut bbox area
    sub_depth = depth[sub_box[1]: sub_box[3], sub_box[0]: sub_box[2]].copy()
    obj_depth = depth[obj_box[1]: obj_box[3], obj_box[0]: obj_box[2]].copy()
    uni_depth = depth[uni_box[1]: uni_box[3], uni_box[0]: uni_box[2]].copy()

    # resize
    sub_depth.resize((depth_size, depth_size))
    obj_depth.resize((depth_size, depth_size))
    uni_depth.resize((depth_size, depth_size))

    return sub_depth, obj_depth, uni_depth
