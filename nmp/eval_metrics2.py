...
def eval_result(test_roidb, pred_roidb, N_recall, is_zs=False, mode='pred', topk=False, dataset='vrd', vis=False):
	...
	if vis:
		if topk:
			...
			with open("/media/DataDrive/xiaochen/11777-Project-VRD/nmp_res_rec%d_top70.pkl"%(n_re), 'wb') as f:
				pickle.dump(output, f, protocol=pickle.HIGHEST_PROTOCOL)
		else:
			# save results for top1+recall50
			with open("/media/DataDrive/xiaochen/11777-Project-VRD/nmp_res_rec%d_top1.pkl"%(n_re), 'wb') as f:
				pickle.dump(output, f, protocol=pickle.HIGHEST_PROTOCOL)
