...
parser.add_argument('--dis', type=float, default=0.45,
                    help='Use motion capture data loader.')
parser.add_argument('--use-loc', action='store_true', default=True,
                    help='use location coordinates')
parser.add_argument('--uni-depth', action='store_true', default=False,
                    help='uni depth information')
parser.add_argument('--obj-depth', action='store_true', default=True,
                    help='obj depth information')
...
if args.encoder == 'nmp':
    model = NMPEncoder(args.hidden,
                       ...
                       do_prob=args.dropout, use_vis=use_vis, use_spatial=use_spatial, use_sem=use_sem, use_loc=args.use_loc, use_cls=args.use_cls, uni_depth=args.uni_depth, obj_depth=args.obj_depth)
...
def iter_one_epoch(roidb, data_loader, batch_size, is_rela=False, is_training=True):
    ...
    for batch_idx, (data, target, node_cls, edge_feats, rel_rec, rel_send, bbox_loc, prior, depth) in enumerate(data_loader):
        if args.cuda:
        ...
        # --------- Forward -----------#
        output, node_output = model(data, edge_feats, rel_rec, rel_send, bbox_loc, depth)
...
def eval(roidb, test_loader, is_rela=False):
    ...
    for batch_idx, (data, target, node_cls, edge_feats, rel_rec, rel_send, bbox_loc, prior, depth) in enumerate(test_loader):
        if args.cuda:
            data, target, edge_feats = data.cuda(), target.cuda(), edge_feats.cuda()
            rel_rec, rel_send = rel_rec.cuda(), rel_send.cuda()
            bbox_loc, prior = bbox_loc.cuda(), prior.cuda()
